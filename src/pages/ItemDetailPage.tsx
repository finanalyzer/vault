...
export default function ItemDetailPage() {
  ...
  // Matches AddBinaryCommand → AddBinaryField pattern from ItemDetailViewModel
  ...
  const handleFileUpload = async (file: File) => {
    ...
    setIsUploading(true);
    try {
      // Matches AddBinaryField: adds binary to the entry, refreshes Fields/attachments collection
      await uploadAttachment(entryId!, file);
      setNotification({ type: 'success', message: t('common.upload') + ' ' + t('common.success') });
      setTimeout(() => setNotification(null), 3000);
      // Refresh attachments list after upload (matches Fields.Add + UpdateItem pattern)
      ...
      <div className="flex-1 flex flex-col">
        <header className="bg-white dark:bg-dark-800 border-b border-light-200 dark:border-dark-600 px-6 lg:px-6 py-4 pl-20 lg:pl-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              {/* Matches ToolbarItem with Paperclip icon bound to AddBinaryCommand in XAML */}
              <div className="relative">
                <Button variant="secondary" onClick={handleAttachClick} disabled={isUploading}>
                  {isUploading ? (
                  ...
                  <span className="hidden md:inline">{t('common.attach')}</span>
                </Button>
                {/* Matches DisplayActionSheet with File/Camera/Gallery options */}
                {showAttachMenu && (
                  <div
                    className="absolute right-0 top-full mt-1 z-50 bg-white dark:bg-dark-800 rounded-lg shadow-lg border border-light-200 dark:border-dark-600 py-1 min-w-[160px]"
                    onClick={(e) => e.stopPropagation()}
                  >
                    {/* Matches field_id_file option */}
                    <button
                    ...
                    {/* Matches field_id_gallery option */}
                    <button
                    ...
                    {/* Matches field_id_camera option */}
                    <button
              ...
              {/* Hidden file inputs matching MediaPicker/FilePicker pattern */}
              <input
                ref={fileInputRef}
                type="file"
                className="hidden"
        ...
        <main className="flex-1 p-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {/* Attachments section - always visible matching MAUI Fields ListView pattern */}
            <div className="bg-white dark:bg-dark-800 rounded-2xl shadow-light-5 dark:shadow-dark-5 overflow-hidden">
              <div className="p-6 border-b border-light-200 dark:border-dark-600 flex items-center justify-between">
                <h2 className="text-lg font-semibold text-light-900 dark:text-light-100">
                  {t('common.attachments')}
                </h2>
                <Button variant="secondary" size="sm" onClick={handleAttachClick} disabled={isUploading}>
                  {isUploading ? (
                    <svg className="animate-spin h-4 w-4 inline mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
              ...
              <div className="divide-y divide-light-100 dark:divide-dark-700">
                {attachments.length === 0 ? (
                  <div className="px-6 py-8 text-center text-light-500 dark:text-dark-400">
                    ...
                    <p className="text-sm">{t('common.noAttachments')}</p>
                  </div>
                ) : (
                  attachments.map((attachment) => (
                    <div
                      key={attachment.id}
                      className={`flex items-center gap-4 px-6 py-4 hover:bg-light-50 dark:hover:bg-dark-700 ${attachment.isImage ? 'cursor-pointer' : ''}`}
                      onClick={() => {
                        // Matches OnFieldSelected: only preview if IsBinaries + Image type
                        if (attachment.isImage) {
                          handlePreviewAttachment(attachment.id);
                        }
                      }}
                    >
