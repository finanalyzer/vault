...
export default function Breadcrumbs({ groupId, groupName, rootGroupName, rootGroupId, parentGroup }: BreadcrumbsProps) {
  const navigate = useNavigate();

  const normalizedGroupId = groupId === 'root' ? rootGroupId : groupId;

  const breadcrumbs: BreadcrumbItem[] = normalizedGroupId === rootGroupId
    ? [{ id: rootGroupId, name: groupName }]
    : parentGroup && parentGroup.id !== rootGroupId && parentGroup.id !== normalizedGroupId
    ? [
        { id: rootGroupId, name: rootGroupName },
        parentGroup,
        { id: normalizedGroupId, name: groupName },
      ]
    : [
        { id: rootGroupId, name: rootGroupName },
        { id: normalizedGroupId, name: groupName },
  ...
  return (
    <nav className="flex items-center gap-1 mt-1 overflow-x-auto whitespace-nowrap">
      {breadcrumbs.map((crumb, index) => (
        <div key={`${index}-${crumb.id}`} className="flex items-center">
          {index > 0 && (
